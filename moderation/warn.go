...
func (c *warn) execute() {
	// If there are no mentions for warning users, bail out.
	...
	// Check if the users to warn have a higher role than the bot.
	...
	userHasPerms, err1 := utility.HasPermissionOr(c.connection, c.message.Author.ID, c.message.ChannelID, discordgo.PermissionBanMembers, discordgo.PermissionKickMembers)
	botHasPerms, err2 := utility.HasPermissionOr(c.connection, c.message.Author.ID, c.message.ChannelID, discordgo.PermissionBanMembers, discordgo.PermissionKickMembers)
	...
	if !userHasPerms {
		_, _ = c.connection.ChannelMessageSend(c.message.ChannelID, ":exclamation: | You require BAN or KICK permissions to perform this action")
		return
	}

	// If the bot does not have permission, send message and bail out.
	if !botHasPerms {
		_, _ = c.connection.ChannelMessageSend(c.message.ChannelID, ":exclamation: | The bot requires BAN or KICK permissions to perform this action")
		return
	}
	...
	// The menu title will be the list of users being warned as well as a message to cancel the warn.
	var menuTitle string = "Moderation menu - Warn\n------------------------------\n" +
		"Users to warn: " + c.modData.Mentions[0].Username + "#" + c.modData.Mentions[0].Discriminator
...
func (c *warn) handleConfirmation() {
	// Handle cancel and bail out if cancelled.
	if c.handleCancel() {
		return
	}
	...
	// Input is 1.
	// For each user, create mod log, send mod log and store in database.
	...
	// Update menu.
	if warnCount == len(c.modData.Mentions) {
		c.updateMenu("\n> Warn successful")
	} else if warnCount > 0 {
		c.updateMenu("\n> Warn partially successful")
	} else {
		c.updateMenu("\n> Warn failed")
	}
}
...
func (c *warn) updateMenu(content string) {
